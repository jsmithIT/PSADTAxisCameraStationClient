...
Try {
	...
	[string]$appVendor = 'Axis'
	[string]$appName = 'AXIS Camera Station Client'
	[string]$appVersion = '5.44.381'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.1.3'
	[string]$appScriptDate = '2022.06.03'
	[string]$appScriptAuthor = 'KTEUTON, SEWALT, CUBAKER, JBSMITH'
	...
	$CloseApps = "AcsClient=AXIS Camera Station"
	...
	$64bitAppInstallName = "AXIS Camera Station"
	# Application install Path.
	$64bitAppInstallPath = "$PSScriptRoot\Files\AXISCameraStationSetup.exe"
	# Application  install parameters.
	$64bitAppInstallParam = "/VERYSILENT /NORESTART /SUPPRESSMSGBOXES /EULA=ACCEPT /TYPE=CLIENT" 
	...
	#$RemoveAppNamesMSI = @("")
	$RemoveAppNamesEXE = @("AXIS Camera Station")

	##* Application uninstall parameters.
	# 64-bit application
	# 64-bit application uninstall name.
	$64bitAppUninstallName = "AXIS Camera Station"
	# Application uninstall path.
	$64bitAppUninstallPath = "$Env:Programfiles\Axis Communications\AXIS Camera Station\unins000.exe"
	# Application uninstall parameters.
	$64bitAppUninstallParam = "/verysilent /suppressmsgboxes /norestart"
	...
	# List of shortcuts to be removed
	[string[]]$RemoveAppShortcuts = @("AXIS Camera Station Client.lnk")

	##* Active Setup 
	# Directory where settings for application will be.
	$asSettingsDir = "$Env:ProgramData\Tuolumne County\Application Settings"

	# Name of the active setup folder.
	$asFolderName = "AXIS Camera Station Client"

	# Name of the file to be ran with active setup.
	$asFileName = "AXIS_Camera_Station_Client-Settings.EXE"

	# Active setup key name.
	$asKeyName = "AXIS Camera Station Client - Settings"

	# Description in active setup.
	$asDescription = "AXIS Camera Station Client - Settings"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		# Check if there is a pending restart and not running in a task sequence.
		# If there is a pending restart, prompt the user to restart the device.
		If (((Get-PendingReboot).IsSystemRebootPending -eq $True) -And ($runningTaskSequence -eq $False)) {
		...
		If (Test-Path -Path "HKLM:\SOFTWARE\Microsoft\Active Setup\Installed Components\$asKeyName") {
			# Remove previous versions of activesetup key.    
			Try {
				Write-Log -Message "Previous versions of activesetup key exist. Removing..." 
				Set-ActiveSetup -Key "$asKeyName" -PurgeActiveSetupKey
			}
			# If removal failed, exit script. Log results. 
			Catch [System.Exception] {
		...
		# Removes previous versions of activesetup that were named differently.
		# Check if previous versions of activesetup key exist.
		# If activesetup key exists, remove previous versions of activesetup key.
		# If removal failed, exit script. Log results.
		If (Test-Path -Path "HKLM:\SOFTWARE\Microsoft\Active Setup\Installed Components\AXIS Camera Station") {
			# Remove previous versions of activesetup key.    
			Try {
				Write-Log -Message "Previous versions of activesetup key exist. Removing..." 
				Set-ActiveSetup -Key "AXIS Camera Station" -PurgeActiveSetupKey
			}
			# If removal failed, exit script. Log results. 
			Catch [System.Exception] {
				Write-Log -Message "Removing of previous versions of activesetup key failed with error: $_."
		...
		# Else, log results from check. 
		Else {  
			Write-Log -Message "Previous versions of activesetup key (named differently) do not exist."        
		}
		...
		# If removal failed, log results. Exit script.
		If (Test-Path -Path "$asSettingsDir\AXIS Camera Station*") {
			# Remove previous versions of settings directory.
			Try {
				Write-Log -Message "Previous versions of settings directory exist. Removing..." 
				Remove-File -Path "$asSettingsDir\AXIS Camera Station*" -Recurse
		...
		# NOTE: Removes previous application shortcuts.
		# For each item in the array...
		# Remove previous application shortcuts in directories.
		# If removal failed, log results. Exit script.
		Foreach ($RemoveAppShortcut in $RemoveAppShortcuts) {
			# Remove previous application shortcuts in directories.
			Try {
				Write-Log -Message "Removing $RemoveAppShortcut from $envCommonDesktop."
				Remove-File -Path "$envCommonDesktop\$RemoveAppShortcut"
				Write-Log -Message "Removing $RemoveAppShortcut from $envCommonDesktop complete."
			}
			# If removal failed, log results. Exit script.
			Catch [System.Exception] {
				Write-Log -Message "Removing $RemoveAppShortcut from $envCommonDesktop failed with error: $_."
				Write-Log -Message "Exiting script with error."
				Exit-Script -ExitCode 1627
			}
		}
		...
		# If creation failed, log results. Exit script.
		Try {
			Write-Log -Message "Creating public desktop shortcut" 
			New-Shortcut -Path "$envCommonDesktop\AXIS Camera Station Client.lnk" -TargetPath "$envProgramFiles\Axis Communications\AXIS Camera Station\Client 5.44.381\AcsClient.exe" -WorkingDirectory "$envProgramFiles\Axis Communications\AXIS Camera Station\Client 5.44.381\" -IconLocation "$envProgramFiles\Axis Communications\AXIS Camera Station\Client 5.44.381\AcsClient.exe" -Description "AXIS Camera Station Client"
			Write-Log -Message "Creating public desktop shortcut complete."
		}
		# If creation failed, log results. Exit script. 
		Catch [System.Exception] {
			Write-Log -Message "Creating public desktop shortcut failed with error: $_."
			Write-Log -Message "Exiting script with error."
			Exit-Script -ExitCode 1627
		}
		...
		Try {
			...
			Copy-File -Path "$dirSupportFiles\$asFolderName" -Destination "$asSettingsDir" -Recurse
		}
		# If copy failed, log results. Exit script.
		Catch [System.Exception] {
		...
		Try {
			...
			Set-ActiveSetup -StubExePath "$asSettingsDir\$asFolderName\$asFileName" -Description "$asDescription" -Key "$asKeyName"
